    ...
    WindowBase* window, const char* windowName, int width, int height, bool isMaximized, bool noResize)
{
	...
	window->m_implData->window =
	    SDL_CreateWindow(windowName, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, wndFlags);
...
void ApplicationHandler::PollEvents()
{
	...
	// they are just generated based on the device that captures the touches (remember touches are not generated by only
	// touch screen, they could be touch pads, pen tablets or other). However, if the SDL hint
	// SDL_HINT_TOUCH_MOUSE_EVENTS is set to "1", SDL somehow manages to convert those touch events to mouse clicks at
	// the correct location. The code basically searches for the focused window and uses its size to rescale the touch
	// inputs from normalized space (called UV space in SGEEngine) to screen pixels. The code below does just that.
			...
			case SDL_WINDOWEVENT: {
				...
				if (wnd) {
					if (event.window.event == SDL_WINDOWEVENT_SHOWN) {
						...
						// changing the window size; this event is followed by SDL_WINDOWEVENT_RESIZED if the size was
						...
						removeWindow(wnd); // There might be some more messages here but there is no window destroyed
						                   // event in SDL.
					}
				}
			} break;
	...
	for (WindowBase* const wnd : m_wnds) {
		const bool wasActiveWhilePolling =
		    true; // SDL_GetWindowFlags(wnd->m_implData->window) & SDL_WINDOW_INPUT_GRABBED;

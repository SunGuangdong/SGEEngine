...
bool ShadingProgramPermuator::createFromFile(
    SGEDevice* sgedev,
	...
	*this = ShadingProgramPermuator();
	...
	if (FileReadStream::readTextFile(filename, fileContents)) {
		...
		return createInternal(
		    ...
		    fileContents.data(),
		    filename,
		    precompiledCacheFilePrefix,
		    compileTimeOptions,
		    uniformsToCacheInLUT,
		    outIncludedFiles);
	}
...
bool ShadingProgramPermuator::create(
    SGEDevice* sgedev,
...
bool ShadingProgramPermuator::createInternal(
    SGEDevice* sgedev,
    const char* const shaderCode,
    const char* const shaderCodeFileName,
    std::string precompiledCacheFilePrefix,
    ...
    std::set<std::string>* outIncludedFiles)
{
	...
	if (kSgeRendApi == SGERendApi_D3D && !precompiledCacheFilePrefix.empty()) {
		precompiledCacheFilePrefix += ".d3d";
	}
	else {
		// Force shader cache to be used only with D3D, it is not implemented for other APIs.
		precompiledCacheFilePrefix.clear();
	}

	if (!precompiledCacheFilePrefix.empty()) {
		isCacheLoaded = programCompiledCache.loadCacheFile(precompiledCacheFilePrefix.c_str());
	}
	...
	if (isCacheLoadedAndValid) {
		...
		if (!precompiledCacheFilePrefix.empty()) {
			sgeLogWarn(
			    "For %s no valid cache file is found. The shaders are going to be compiled.", shaderCodeFileName);
		}
	...
	for (int iPerm = 0; iPerm < compileTimeOptionsPermutator.getAllPermunations().size(); ++iPerm) {
		...
		for (int t = 0; t < uniformsToCacheInLUT.size(); ++t) {
			BindLocation bindLoc =
			    refl.findUniform(uniformsToCacheInLUT[t].uniformName, uniformsToCacheInLUT[t].shaderStage);
	...
	if (!hadErrors && !isCacheLoadedAndValid && !precompiledCacheFilePrefix.empty()) {
		generateShadingProgramsCompilationCache(precompiledCacheFilePrefix.c_str());
	}

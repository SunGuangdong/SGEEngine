...
/// A widget that aligns other widgets in a row.
...
struct SGE_CORE_API RowLayout final : public IWidget {
	RowLayout(UIContext& owningContext)
  ...
  public:
	...
	Unit spacingX;     ///< The horizontal spacing between widgets.
	Unit spacingY;     ///< The vertical spacing between widgets (if multiple lines are needed).
	...
	bool breakInMultipleLines = true;

...
namespace PathLengthFollow {
	struct SGE_ENGINE_API Settings {
		bool isReversed = false; // true if the movement starts form the end of the path. (the includes the distance
	...
	struct SGE_ENGINE_API State {
		...
		float distanceAlongPath = 0.f; // The distance travelled in the paths object space. the value is maintained the
		                               // same if reversed is specified to the settings, the only thing that is changed
		                               // is that the starting point is now the end point.
	...
	/// @param recursionDepth Also leave default value. Used to prevent endless recursion if there is a mistake inside
	/// the function.
	SGE_ENGINE_API State compute(
...
namespace PathFollow {
	...
	/// @param recursionDepth Also leave default value. Used to prevent endless recursion if there is a mistake inside
	/// the function.
	SGE_ENGINE_API Optional<State>
	    compute(const float dt, GameWorld* const world, const Settings& settings, const State& prevState);
}; // namespace PathFollow

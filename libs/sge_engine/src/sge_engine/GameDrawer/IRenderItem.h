...
/// Imagine a scene where we have an object inside a semi-transparent sphere - like the one in the Super Monkey Ball
/// games. Imagine that "Mokey Ball" is one FBX file. As we've said the ball is semi transparent, in order to to the
/// alpha blending correctly we must take all different parts of an object (in the case of the mokey ball the monkey and
/// the semi-transparent ball) separate the opaque from semi-transparent, and then sort semi-transparent in
/// back-to-front order (relative to the camera).
///
...
/// break the actual fooliage into smaller cunks and have one render item for each chunk - by doing this we will get
/// frustum culling.
struct IRenderItem {
	IRenderItem() = default;

...
/// @param [in] codeFilename is the name of the file that contains the code (could be anything really that fits the
/// command line).
/// @param [in] macros an arrays of marcos to be specified (may be nullptr).
/// @param [in] numMacros the array size of @macros.
/// @param [in] includeDir the include directory to be used when resolving #include macros (TODO: make this accept more
/// that one directory).
/// @param [out] outIncludedFiles optional list of all included files during the parsing, excluding @codeFilename if not
/// included explicitly.
